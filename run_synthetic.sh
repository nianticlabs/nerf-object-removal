...
sed -i.bak \
  ...
  model/configs/custom_synthetic/default.gin
sed -i.bak \
...
# OUTPUT_DIR=/mnt/res_nas/silvanweder/experiments/final_tests_synthetic_v3
HOME_DIR=$(pwd)
...
python preprocessing/convert_colmap_to_nerf_synthetic.py \
...
cd external/lama
bash run.sh "${OUTPUT_DIR}/${SCENE}/data" "${OUTPUT_DIR}" "${SCENE}" '_synthetic'
cd ../..
