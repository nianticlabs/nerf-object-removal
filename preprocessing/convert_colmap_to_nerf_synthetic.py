...
def main(args):
    ...
    test_end = start_stop['synthetic_end']
    ...
    synthetic_masks = sorted(os.listdir(os.path.join(sequence_path, 'masks_synthetic')))
    if test_end > -1:
        print(test_end)
        print(len(synthetic_masks))
        synthetic_masks = synthetic_masks[:test_end-2]
        print(len(synthetic_masks))
    ...
    if args.split:
        ...
        for k, v in out.items():
            ...
            if k == 'frames':
                train_split[k] = []
                test_split[k] = []
                # filter masks if they belong to synthetic benchmark
                frame_count = 0
                for f_ in v:
                    if f_['file_path'].split('/')[-1].replace('.jpg', '.npy') in synthetic_masks:
                        if frame_count % 8 == 0:
                            test_split[k].append(f_)
                        else:
                            train_split[k].append(f_)
                        frame_count += 1
                # train_split[k] = v[:start_end_meta['split_end']]
