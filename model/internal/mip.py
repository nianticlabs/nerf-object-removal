...
# Original file: https://github.com/google-research/google-research/blob/master/regnerf/internal/mip.py
...
def compute_alpha_weights(density, t_vals, dirs, invert=False, box=None, samples=None):
  ...
  if box is not None:
    ...
    box_mask = (samples[:, :, 0] >= box[0, 0]) & (samples[:, :, 0] < box[0, 1]) & \
               (samples[:, :, 1] >= box[1, 0]) & (samples[:, :, 1] < box[1, 1]) & \
               (samples[:, :, 2] >= box[2, 0]) & (samples[:, :, 2] < box[2, 1])
    alpha = alpha * box_mask


  trans = jnp.concatenate((jnp.zeros_like(alpha[Ellipsis, :1]), alpha[Ellipsis, :-1]), axis=-1)
  trans = jnp.cumprod(1. - trans, axis=-1)
...
def volumetric_rendering(rgbs, weights, normals, beta, alpha, t_vals, white_background,
                         vis_num_rays, compute_extras, delta, alpha_weights, beta_min=0.03, alpha_min=0.03, 
                         label=None, weights_label=None, delta_label=None, features=None, normalize_weights=False):
  ...
  if normalize_weights:
    weights_ = weights / acc[Ellipsis, None]
    rgb = (weights_[Ellipsis, None] * rgbs).sum(axis=-2)
    rgb_no_density_grad = (jax.lax.stop_gradient(weights_[Ellipsis, None]) * rgbs).sum(axis=-1)
  
  else:
    rgb = (weights[Ellipsis, None] * rgbs).sum(axis=-2)
    rgb_no_density_grad = (jax.lax.stop_gradient(weights[Ellipsis, None]) * rgbs).sum(axis=-2)

  # code required for entropy regularization
  ray_density = (alpha_weights / (alpha_weights.sum(axis=-1, keepdims=True) + 1.e-6)) + 1.e-06 # the two adds are to prevent NaNs in the loss
  ray_entropy = -1. * jnp.sum(ray_density * jnp.log(ray_density), axis=-1)
  ray_entropy_mask = jnp.where(alpha_weights.sum(axis=-1) > 0.1, jnp.ones_like(ray_entropy), jnp.zeros_like(ray_entropy))
  
  if white_background:
    rgb = rgb + (1. - acc[Ellipsis, None])

  rendering = {'rgb': rgb,
               'rgb_no_density_grad': rgb_no_density_grad}
  
  rendering['ray_entropy'] = ray_entropy
  rendering['ray_entropy_masked'] = ray_entropy * ray_entropy_mask

  if label is not None:
    rendering['label_pointwise'] = label.copy()
  
  rendering['alpha_pointwise'] = alpha_weights
  rendering['weights_pointwise'] = weights
  rendering['t_vals_pointwise'] = t_vals
  # volume render model uncertainty
  beta_map = (weights * beta).sum(axis=-1)
  rendering['betas'] = beta_map + beta_min

  # volume render data uncertainty
  alpha_map = (weights * alpha).sum(axis=-1)
  rendering['alphas'] = alpha_map + alpha_min

  if features is not None:
    feature_map = (weights[Ellipsis, None] * features).sum(axis=-2)
    rendering['features'] = feature_map
  ...
  if compute_extras:
    ...
    # rendering['distance_mean'] = (weights * t_mids).sum(axis=-1)
    ...
    if label is not None:
      label_map = (weights_label[Ellipsis, None] * label).sum(axis=-2)
      rendering['label'] = label_map

    rendering['distance_mean'] = (
        jnp.clip(jnp.nan_to_num(expectation(t_mids), jnp.inf), t_vals[:, 0], t_vals[:, -1]))
...
def volumetric_rendering_two_head(rgbs_view_dir, rgbs_multi_view, weights, normals, t_vals, white_background,
                                 vis_num_rays, compute_extras, delta, alpha_weights, normalize_weights=False):
  """Volumetric Rendering Function.
  ...
  if normalize_weights:
    weights_ = weights / acc[Ellipsis, None]
    rgb_view_dir = (weights_[Ellipsis, None] * rgbs_view_dir).sum(axis=-2)
    rgb_no_density_grad_view_dir = (jax.lax.stop_gradient(weights_[Ellipsis, None]) * rgbs_view_dir).sum(axis=-1)
    rgb_multi_view = (jax.lax.stop_gradient(weights_[Ellipsis, None]) * rgbs_multi_view).sum(axis=-2)
    rgb_no_density_grad_multi_view = (jax.lax.stop_gradient(weights_[Ellipsis, None]) * rgbs_multi_view).sum(axis=-1)
  
  else:
    rgb_view_dir = (weights[Ellipsis, None] * rgbs_view_dir).sum(axis=-2)
    rgb_no_density_grad_view_dir = (jax.lax.stop_gradient(weights[Ellipsis, None]) * rgbs_view_dir).sum(axis=-2)
    rgb_multi_view = (weights[Ellipsis, None] * rgbs_multi_view).sum(axis=-2)
    rgb_no_density_grad_multi_view = (jax.lax.stop_gradient(weights[Ellipsis, None]) * rgbs_multi_view).sum(axis=-2)
  ...
  rendering = {'rgb_view_dir': rgb_view_dir,
               'rgb_view_dir_no_density_grad': rgb_no_density_grad_view_dir,
               'rgb_multi_view': rgb_multi_view,
               'rgb_multi_view_no_density_grad': rgb_no_density_grad_multi_view}
  ...
  if compute_extras:
    ...
    t_vals_flat = t_vals.reshape([-1, t_vals.shape[-1]])
    weights_flat = weights.reshape([-1, weights.shape[-1]])
    rgbs_flat = rgbs_multi_view.reshape([-1] + list(rgbs_multi_view.shape[-2:]))
    ray_entropy = -weights / delta * jnp.log(
        jnp.clip(weights / jnp.clip(delta, eps), eps, 1 - eps))
    ray_entropy = ray_entropy.reshape([-1, ray_entropy.shape[-1]])
    rendering['ray_t_vals'] = t_vals_flat[:vis_num_rays, :]
