...
# Original file: https://github.com/google-research/google-research/blob/master/regnerf/internal/models.py
...
class MipNerfModel(nn.Module):
  ...
  density_coupling: bool = False
  mlp_model: str = 'unc'
  normalize_weights: bool = False
  view_uncertainty: bool = False
  view_uncertainty_code:  bool = True
  num_mlp_model_heads: int = 1
  ...
  def __call__(
    ...
    if self.num_mlp_model_heads == 1:
      mlp = OneHeadMLP()
    elif self.num_mlp_model_heads == 2:
      mlp = TwoHeadMLP()
    ...
    for i_level in range(self.num_levels):
      ...
      if self.num_mlp_model_heads == 1:
        if self.use_viewdirs:
          (rgb, density, normals, beta, alpha, label) = mlp(rng, samples, rays.viewdirs, self.label_mask)
        else:
          (rgb, density, normals, beta, alpha, label) = mlp(rng, samples, None, self.label_mask)
      
      
      
      elif self.num_mlp_model_heads == 2:
          (rgb, rgb_multi_view, density, normals) = mlp(rng, samples, rays.viewdirs, self.label_mask)

  
      # Volumetric rendering.
      weights, alpha_weights, _, delta = mip.compute_alpha_weights(
          density, t_vals, rays.directions, samples=None)
      
      if self.num_mlp_model_heads == 1:
        rendering = mip.volumetric_rendering(
              rgb,
              weights if not stop_gradient_density else jax.lax.stop_gradient(weights),
              normals,
              beta,
              alpha,
              t_vals,
              ...
              compute_extras,
              delta,
              alpha_weights if not stop_gradient_density else jax.lax.stop_gradient(alpha_weights),
              beta_min=self.beta_min,
              alpha_min=self.alpha_min,
              label=label,
              weights_label=jax.lax.stop_gradient(weights),
              delta_label=delta,
              normalize_weights=self.normalize_weights
        )

      elif self.num_mlp_model_heads == 2:
        rendering = mip.volumetric_rendering_two_head(
              rgb,
              rgb_multi_view,
              weights if not stop_gradient_density else jax.lax.stop_gradient(weights),
              normals,
...
class TwoHeadMLP(nn.Module):
  """A two head MLP."""
  net_shared_depth: int = 8
  net_color_depth: int = 4
  net_label_depth: int = 4
  ...
  net_depth_alpha: int = 1  # The depth of the second part of MLP.
  net_width_alpha: int = 128  # The width of the second part of MLP.
  ...
  beta_activation: Callable[Ellipsis, Any] = nn.softplus
  alpha_activation: Callable[Ellipsis, Any] = nn.softplus
  ...
  disable_normals: bool = True  # If True, don't bother computing normals.
  use_viewdirs_semantic: bool = True

  @nn.compact
  def __call__(self, rng, samples, viewdirs=None, label_mask=False):
    """Evaluate the MLP.
    ...
    def predict_density(rng, means, covs):
      ...
      # Comment Silvan: the density bias is there to get the shifted softplus in order to 
      # initially get low density everywhere.
    ...
    # first head (view dependent color)
    # predict color based on embedding
    ...
    x_color_view_dir = jnp.concatenate([bottleneck, viewdirs_enc], axis=-1)

    # Here use 1 extra layer to align with the original nerf model.
    for _ in range(self.net_depth_viewdirs):
      x_color_view_dir = dense_layer(self.net_width_viewdirs)(x_color_view_dir)
      x_color_view_dir = self.net_activation(x_color_view_dir)

    # second head (mulit view color)
    x_color_multi_view = bottleneck

    # Here use 1 extra layer to align with the original nerf model.
    for _ in range(self.net_depth_viewdirs):
      x_color_multi_view = dense_layer(self.net_width_viewdirs)(x_color_multi_view)
      x_color_multi_view = self.net_activation(x_color_multi_view)

    # final layer
    rgb_view_dir = self.rgb_activation(dense_layer(self.num_rgb_channels)(x_color_view_dir))
    rgb_multi_view = self.rgb_activation(dense_layer(self.num_rgb_channels)(x_color_multi_view))
    
    rgb_view_dir = rgb_view_dir * (1 + 2 * self.rgb_padding) - self.rgb_padding
    rgb_multi_view = rgb_multi_view * (1 + 2 * self.rgb_padding) - self.rgb_padding

    # normals dummy as we are not computing it at the moment
    normals = jnp.full_like(means, fill_value=jnp.nan)

    return (rgb_view_dir, rgb_multi_view, density, normals)
...
class OneHeadMLP(nn.Module):
  ...
  def __call__(self, rng, samples, viewdirs=None, label_mask=False):
    ...
    def predict_density(rng, means, covs):
      ...
      raw_density = dense_layer(1)(x)[Ellipsis, 0]  # Hardcoded to a single channel.
      raw_beta = dense_layer(1)(x)[Ellipsis, 0]
    ...
    if viewdirs is not None:
      ...
      x_color = jnp.concatenate([bottleneck, viewdirs_enc], axis=-1)
      x_label = bottleneck.copy()

      # Here use 1 extra layer to align with the original nerf model.
      for _ in range(self.net_depth_viewdirs):
        x_color = dense_layer(self.net_width_viewdirs)(x_color)
        x_color = self.net_activation(x_color)
    ...
    for _ in range(self.net_depth_viewdirs):
      x_label = dense_layer(self.net_width_viewdirs)(x_label)
      x_label = self.net_activation(x_label)

    rgb = self.rgb_activation(dense_layer(self.num_rgb_channels)(x_color))
    rgb = rgb * (1 + 2 * self.rgb_padding) - self.rgb_padding
    
    label = dense_layer(1)(x_label)
  
    # normals dummy as we are not computing it at the moment
    normals = jnp.full_like(means, fill_value=jnp.nan)
    # other dummys
    beta_color = jnp.zeros_like(density)
    alpha = jnp.zeros_like(density)

    return (rgb, density, normals, beta_color, alpha, label)
