...
def visualize_labels(labels, cmap):
...
def visualize_suite(rendering, rays, config):
  ...
  rgb_view_dir = rendering['rgb_view_dir']
  rgb_multi_view = rendering['rgb_multi_view']
  ...
  rgb_matte = matte(rgb_multi_view, acc)

  vis_mean, vis_median = [
      visualize_cmap(x, jnp.ones_like(acc), cm.get_cmap('turbo'), curve_fn=depth_curve_fn)
      for x in [distance_mean, distance_median]
  ]

  vis_mean_mod, vis_median_mod = [
      visualize_cmap(x, jnp.ones_like(acc), sinebow, modulus=0.1, curve_fn=depth_curve_fn)
      for x in [distance_mean, distance_median]
  ]
  ...
  vis = {
      'color_view_dir': rgb_view_dir,
      'color_multi_view': rgb_multi_view,
      'acc': acc,
      'normals': normals,
