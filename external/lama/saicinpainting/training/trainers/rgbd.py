...
from saicinpainting.training.trainers.rgbd_base import RGBDBaseInpaintingTrainingModule, make_multiscale_noise
...
class RGBDInpaintingTrainingModule(RGBDBaseInpaintingTrainingModule):
    ...
    def forward(self, batch):
        if self.training and self.rescale_size_getter is not None:
            ...
            batch['depth'] = F.interpolate(batch['depth'], size=cur_size, mode='bilinear', align_corners=False)
        ...
        if self.concat_mask:
            masked_img = torch.cat([masked_img, masked_depth, mask], dim=1)

        prediction = self.generator(masked_img)
        batch['predicted_image'] = prediction[:, :3]
        batch['predicted_depth'] = prediction[:, -1].unsqueeze(1)
        batch['inpainted'] = mask * batch['predicted_image'] + (1 - mask) * batch['image']
        batch['inpainted_depth'] = mask * batch['predicted_depth'] + (1 - mask) * batch['depth']
        
        #TODO: double check that to also have fakes for depth
        if self.fake_fakes_proba > 1e-3:
    ...
    def generator_loss(self, batch):
        ...
        predicted_depth = batch['predicted_depth']

        # L1
        l1_value_rgb = masked_l1_loss(predicted_img, img, supervised_mask,
                                      self.config.losses.l1.weight_known,
                                      self.config.losses.l1.weight_missing)
        l1_value_depth = masked_l1_loss(predicted_depth, depth, supervised_mask,
                                      self.config.losses.l1.weight_known,
                                      self.config.losses.l1.weight_missing)

        l1_value = l1_value_rgb + l1_value_depth
        total_loss = l1_value
