...
class RGBDInpaintingTrainDataset(Dataset):
    def __init__(self, indir, mask_generator, transform, n_samples=0, **kwargs):
        
        self.in_files = list(glob.glob(os.path.join(indir, '**', 'lowres_wide', '*.png'), recursive=True))
        self.in_depth_files = list(glob.glob(os.path.join(indir, '**', 'lowres_depth', '*.png'), recursive=True))
        self.in_depth_files = [os.path.join(*f.split('/')[-3:]) for f in self.in_depth_files]
        
        # filter rgb according to depth
        self.in_files = [f for f in self.in_files if os.path.join(*f.split('/')[-3:]).replace('wide', 'depth') in self.in_depth_files]

        if n_samples > 0:
            self.in_files = random.sample(self.in_files, n_samples)
    ...
    def __getitem__(self, item):
        ...
        # img = self.transform(image=img)['image'] #TODO: fix that for depth + image
        img = img.astype(np.float32) / 255.
        img = np.transpose(img, (2, 0, 1))

        depth = np.asarray(Image.open(path.replace('lowres_wide', 'lowres_depth')))
        depth = np.expand_dims(depth, axis=0).astype(np.float32) / 1000.
...
def make_default_train_dataloader(indir, kind='default', out_size=512, mask_gen_kwargs=None, transform_variant='default',
    ...
    if kind == 'default':
        ...
        dataset = RGBDInpaintingTrainDataset(indir=indir,
                                             mask_generator=mask_generator,
                                             transform=transform,
                                             **kwargs)
    else:
